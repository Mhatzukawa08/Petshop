...
if (isset($_POST['konfirmasi_pesanan'])) {
    ...
    $query = mysqli_query($koneksi, "UPDATE pesanan_produk SET selesai='1', bayar='1' WHERE id_pemesanan='$id_pemesanan' ");

    if ($query) {
        echo '
                <script>
                    alert("Berhasil Konfirmasi Pesanan");
                    window.location.href="riwayat-pesanan-produk.php?id_pesanan_produk=' . $id_pesanan_produk . '";
                </script>
            ';
    } else {
        echo '
