...
if (isset($_POST['tambah_jadwal'])) {
    $id_pesanan_vaksin = $_POST['id_pesanan_vaksin'];
    $tanggal = $_POST['tanggal'];
    $waktu = $_POST['waktu'];
    $jadwal = "$tanggal $waktu";
    $ket_jadwal = $_POST['ket_jadwal'];

    $query = mysqli_query($koneksi, "UPDATE `pesanan_vaksin` SET jadwal='$jadwal', ket_jadwal='$ket_jadwal' WHERE id_pesanan_vaksin='$id_pesanan_vaksin' ");

    if ($query) {
        echo '
                <script>
                    alert("Berhasil Tambah Jadwal");
                    window.location.href="pesanan-vaksin.php?id_pesanan_vaksin=' . $id_pesanan_vaksin . '";
                </script>
            ';
    } else {
        echo '
    ...
    if ($error_image == 0) {
        if ($ekstensi_image == "png" || $ekstensi_image == "jpg" || $ekstensi_image == "jpeg") {
            ...
            $query = mysqli_query($koneksi, "INSERT INTO `dokumentasi` (`id_pilihan_pesanan`, `id_pesanan`, `keterangan`, `gambar`, `waktu`) 
                VALUES ('$id_pilihan_pesanan', '$id_pesanan_vaksin', '$keterangan', '$gambar', '$tanggalDanWaktu') ");
...
if (isset($_POST['konfirmasi_pesanan'])) {
    ...
    $query = mysqli_query($koneksi, "UPDATE pesanan_vaksin SET ket='1', bayar='1' WHERE id_pesanan_vaksin='$id_pesanan_vaksin' ");

    if ($query) {
        echo '
                <script>
                    alert("Berhasil Konfirmasi Pesanan");
                    window.location.href="riwayat-pesanan-vaksin.php?id_pesanan_vaksin=' . $id_pesanan_vaksin . '";
                </script>
            ';
    } else {
        echo '
